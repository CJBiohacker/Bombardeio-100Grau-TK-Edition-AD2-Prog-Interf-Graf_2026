...
class GameView(object):
    ...
    def __init__(self, root, controller):
        ...
        self.root.title("Bombardeio 1000Grau - AD2 Edicao TK (Python 2)")
    ...
    def mostrar_menu(self, globais):
        ...
        tk.Label(frame, text="AD2 - Tkinter Edition (Python 2)", font=self.font_subtitle, bg=self.cores["bg"], fg=self.cores["texto_secundario"]).pack(pady=(0, 40))
        ...
        stats_text = (
            "Partidas Jogadas: {}\n"
            "Média Turnos Sobrevividos: {}\n"
            "Taxa Média de Desmatamento (Global): {}%"
        ).format(
            globais.get('partidas_jogadas', 0),
            globais.get('media_turnos', 0.0),
            globais.get('taxa_destruicao_media', 0.0)
        )
    ...
    def atualizar_hud(self, turno_atual, turnos_limite, total_inimigos, status="EM EXECUÇÃO"):
        text = "Turno: {}/{}  |  Inimigos: {}  |  Status: {}".format(turno_atual, turnos_limite, total_inimigos, status)
    ...
    def desenhar_mapa(self, mapa):
        ...
        self.cell_w = self.canvas_w / float(mapa.colunas)
        self.cell_h = self.canvas_h / float(mapa.linhas)
                ...
                if conteudo is not None:
                    ...
                    if nome_classe == "Obstaculo":
                        if conteudo.tipo == "indestrutivel":
                            self.canvas.create_rectangle(x1+2, y1+2, x2-2, y2-2, fill=self.cores["obstaculo_indestrutivel"], outline="", tags="obs_{}_{}".format(l, c))
                        else:
                            self.canvas.create_rectangle(x1+4, y1+4, x2-4, y2-4, fill=self.cores["obstaculo_destrutivel"], outline="", tags="obs_{}_{}".format(l, c))
    ...
    def mostrar_mensagem_gui(self, titulo, mensagem):
