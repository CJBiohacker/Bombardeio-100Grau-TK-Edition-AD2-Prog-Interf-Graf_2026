...
class GerenciadorEstado:
    ...
    def atualizar_sessao(cls, chave, valor):
        ...
        if chave in dados:
            ...
            print(f"Chave '{chave}' não existe na sessão.")
    ...
    def registrar_fim_partida_atualizacao_global(cls, causa_termino, mapa=None):
        ...
        if total_partidas > 0:
            global_stats["media_turnos"] = round(global_stats["soma_turnos_sobrevividos"] / total_partidas, 2)
            global_stats["media_bombas"] = round(global_stats["soma_bombas_utilizadas"] / total_partidas, 2)
            ...
            if soma_gerados > 0:
                global_stats["taxa_destruicao_media"] = round((soma_destruidos / soma_gerados) * 100, 2)
            else:
